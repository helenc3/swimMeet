...
from scraper.njcom.updates.utils import (
    scrapeOneMeet,
    getonedayurls,
    save_meet_data_to_mongodb,
    TIMEDELTA_BETWEEN_GAMES,
)
...
"""
this is the main script for scraping data from the web.
- uses scrapingUtilsv2 for scraping methods.
- prompts user for date range and additional URLs to scrape.
- no methods here called elsewhere. main method.
"""

CONFERENCES = ["CVC"]


### some helper methods called nowhere else but this script
def prompt_date_range():
    """
    Prompts user for date range. Default is 4 (?) days ago (TIMEDELTA_BETWEEN_GAMES).
