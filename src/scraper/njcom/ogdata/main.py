from scraper.njcom.ogdata.utils import (
    openpage_signin,
    chooseOtherSzn,
    get_divisions,
    chooseDivision,
...
import json
import re
from common.utils import NJCOM_DATA_DIR

"""
main file to scrape nj.com original data
run this one-- if necessary- this is the actually OG to this project- saves data to files
(no mongodb)
---- run archive/jsontodb.py to save to mongodb ----- hopefully one day i will make a cleaner pipeline
"""
...
(NJCOM_DATA_DIR / szn).mkdir(parents=True, exist_ok=True)
...
for team in teams:
    ...
    team_dir = NJCOM_DATA_DIR / szn / team
    ...
    with open(team_dir / "roster.csv", "w", encoding="utf-8", newline="") as f:
        f.write("name,link\n")
    ...
    for name, link in roster:
        ...
        swimmer_dir = swimmers_dir / safe
        swimmer_dir.mkdir(parents=True, exist_ok=True)
        
        # Write nj.com data to njcom.json
        with open(swimmer_dir / "njcom.json", "w", encoding="utf-8") as f:
