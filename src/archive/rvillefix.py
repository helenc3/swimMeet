"""
there seems to be an error with robbinsville courses in the nj.com data, lists as meters where the times rly shouldnt be
...
from archive.course_handler.utils import TEAMCOURSES_FILE
import re
import json
import os
...
def load_team_courses(teamcourses_file=TEAMCOURSES_FILE):
    """Load team courses from JSON file"""
    file_path = Path(teamcourses_file)
    try:
