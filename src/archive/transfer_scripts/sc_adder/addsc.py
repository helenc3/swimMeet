...
from scraper.swimcloud.scutils import open_and_search
...
SC_ADDER_DIR = Path(__file__).resolve().parent
MISSING_SWIMCLOUDS_FILE = SC_ADDER_DIR / "missingswimclouds.json"
SWIMCLOUD_ERRORS_FILE = SC_ADDER_DIR / "errors.json"
...
def add_swimclouds(driver, oldcollection, newcollection, missing_swimclouds_file=MISSING_SWIMCLOUDS_FILE):
    """
    ...
    Args:
        ...
        missing_swimclouds_file: Path to JSON file with missing swimcloud URLs
    """
    ...
    for swimmer in swimmers_to_add:
        ...
        if swimcloud_urls:
            ...
            if result.matched_count > 0:
                print(f"  ipdated {swimmer} with {len(swimcloud_urls)} swimcloud URL(s)")
            else:
    ...
    if swimmers_with_errors:
        with open(SWIMCLOUD_ERRORS_FILE, 'w', encoding='utf-8') as f:
            json.dump(swimmers_with_errors, f, indent=2, ensure_ascii=False)
        print(f"\n⚠ {len(swimmers_with_errors)} swimmer(s) had errors - saved to {SWIMCLOUD_ERRORS_FILE}")
    else:
