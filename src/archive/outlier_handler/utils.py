...
from common.mongodb.queries1 import get_best_time_from_swimmer, get_all_times_for_event, find_document_with_time, get_all_events_for_swimmer
import json
import os
import numpy as np
from pathlib import Path

OUTLIER_HANDLER_DIR = Path(__file__).resolve().parent
IMPOSSIBLE_TIMES_FILE = OUTLIER_HANDLER_DIR / "impossibletimes.json"
OUTLIERS_OUTPUT_DIR = OUTLIER_HANDLER_DIR / "outliers"
IMPOSSIBLE_TIMES_FLAG_FILE = OUTLIER_HANDLER_DIR / "impossible_times_flagged.txt"
...
def process_all_swimmers_outliers(collection, output_dir=OUTLIERS_OUTPUT_DIR, method='iqr', factor=1.5, season="2025-2026"):
    """
...
def load_impossible_times(filename=IMPOSSIBLE_TIMES_FILE):
...
def find_and_handle_impossible_times(
    collection,
    impossible_times_file=IMPOSSIBLE_TIMES_FILE,
    flag_file=IMPOSSIBLE_TIMES_FLAG_FILE,
):
    """
